...
DB = Mongo::Connection.new.db("todo2",:poll_size => 5,:timeout => 5)

get '/' do 
  haml :index, :attr_wrapper => '"', :locals => {:title => 'hello sinatra'}
end

get '/todo' do
  haml :todo, :attr_wrapper => '"',  :locals => {:title => 'Our Sinatra todo app with backbone'}
end
